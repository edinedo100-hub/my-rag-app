...
# ---------------------------------------------------------------------------
# TF-IDF index (cached once per session) — lightweight, ~50 MB total RAM
# ---------------------------------------------------------------------------
...
def build_index():
    ...
    doc_matrix = vectorizer.fit_transform(cleaned)
    return vectorizer, doc_matrix, cleaned


def search(query, top_k=3):
    vectorizer, doc_matrix, cleaned = build_index()
...
def home_page():
    ...
    st.markdown(
        ...
        **international business**. It uses **TF-IDF vector search** with
        cosine similarity to retrieve the passages most related to your
        question.
        """
    )
    ...
    if query:
        ...
        for i, r in enumerate(results, start=1):
            with st.container(border=True):
                st.markdown(
                    f"**Result {i}** — {r['topic']} — similarity: `{r['score']:.4f}`"
                )
                st.write(r["text"])
